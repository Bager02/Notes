    ...
    <div class="bg-gray-100 p-8 rounded-lg shadow-md">
      ...
      <div class="mb-8">
        ...
        <button @click="createNote"
          class="p-4 bg-green-600 text-white font-semibold rounded-md hover:bg-green-700 transition">Create
          Note</button>
      </div>

      <div v-if="store.notes.length > 0">
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          <router-link v-for="note in store.notes" :key="note.id" :to="`/note/${note.id}`"
            ...
            <p class="text-xl font-medium text-gray-900 mb-2">{{ note.title }}</p>
            <p class="text-gray-700 flex-grow overflow-hidden break-words">{{ note.body }}</p>
          </router-link>
        </div>
      </div>
...
const createNote = async () => {
  // Make sure title and body are not empty
  if (newNote.value.title && newNote.value.body) {
    await store.createNote(newNote.value);
    newNote.value.title = ''; // Clear title input after submission
    newNote.value.body = ''; // Clear body input after submission
  } else {
